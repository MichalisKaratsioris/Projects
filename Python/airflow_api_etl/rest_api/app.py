...
try:
    ...
    def products():
        if request.method == 'GET':
            get_query: str = "SELECT * FROM products FOR JSON PATH"
        ...
        if request.method == 'POST':
            if request.headers.get('key') in api_key_list:
                ...
                elif not isinstance(request.form.get('type'), str):
                ...
                insert_query = "INSERT INTO products(name, price, type, featured, visibility, description) VALUES(?, ?, ?, ?, ?, ?)"
    ...
    def query_product_type():
        args = request.args
        type = args.get('type')
        get_query: str = "SELECT * FROM products WHERE type=?"
        ...
        if request.method == 'PUT':
            if request.headers.get('key') in api_key_list:
                ...
                if request.form.get('description') is None:
                    ...
                    return jsonify({"Error": "Description field should be a string."}), 400
                insert_query = f"UPDATE products SET name = ?, price = ?, type = ?, featured = ?, visibility = ?, description = ? WHERE id = {product_id}"
                name = request.form.get('name')
                price = request.form.get('price')
                type = request.form.get('type')
                featured = request.form.get('featured')
                    ...
                    for row in rows:
                        if product_id == row.id:
                            element = {"id": row.id,
                                       ...
                                       "visibility": row.visibility,
                                       "description": row.description}
                            insert_query = f"DELETE FROM products WHERE id = {row.id}"
