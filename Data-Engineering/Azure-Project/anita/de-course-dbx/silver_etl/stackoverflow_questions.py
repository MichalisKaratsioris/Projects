...
#df = spark.read.format("delta").load("/mnt/bronze/b_stackoverflow_questions")
b_df = spark.read.table('bronze_db.b_stackoverflow_questions')
...
if v_file_date == "20220731":
    b_df.write.mode("overwrite").option("overwriteSchema", True).format("delta").saveAsTable("silver_db.s_stackoverflow_questions")

# COMMAND ----------

#use the following modifications to the stackoverflow bronze layer’s etls to 
...
#▪ pick a column and change its value on 1000 rows when it is running with the 
#first two load date parameters (the last, ‘2022-09-30’ dataset should contain the original data in it)
...
if v_file_date == "20220731":
    s_df = spark.read.table("silver_db.s_stackoverflow_questions")
    ...
    s_df.write.mode("overwrite").option("mergeSchema", "true").format("delta").saveAsTable("silver_db.s_stackoverflow_questions")

# COMMAND ----------

# MAGIC %sql
# MAGIC 
# MAGIC MERGE INTO silver_db.s_stackoverflow_questions as s
# MAGIC USING bronze_db.b_stackoverflow_questions as b
# MAGIC 
...
# MAGIC %sql
# MAGIC SELECT * FROM silver_db.s_stackoverflow_questions LIMIT 100
