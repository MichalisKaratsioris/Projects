...
from pyspark.sql.functions import split, col, lit, current_timestamp, to_utc_timestamp, to_date, col, concat
...
silver_df = spark.read.format("delta").load(f"{bronze_folder_path}/b_stackoverflow_post_questions/") \
...
# silver_df = silver_df.loc[:1000, 'score'] = 999999
...
#    .withColumn("dbx_updated_at_datetime_utc", to_utc_timestamp(current_timestamp(),'UTC'))
...
# MAGIC %md
# MAGIC Filtering for testcase:

# COMMAND ----------

# silver_df = silver_df.filter("last_activity_date_datetime_utc > to_date('2022-07-30')") \
#    .withColumn("title", concat(lit(" + Dummy + "),"title"))
...
DeltaTable.createIfNotExists(spark) \
        .tableName("silver_db.s_stackoverflow_post_questions") \
        .addColumn("id", "INT") \
        ...
        .location("dbfs:/mnt/silver/s_stackoverflow_post_questions") \
        .execute()
...
target_df = DeltaTable.forName(spark, "silver_db.s_stackoverflow_post_questions")
...
# MAGIC %sql
# MAGIC 
# MAGIC SELECT title, creation_date_datetime_utc, last_activity_date_datetime_utc, dbx_created_at_datetime_utc, dbx_updated_at_datetime_utc
# MAGIC FROM silver_db.s_stackoverflow_post_questions
# MAGIC -- WHERE valid_date = "2022-08-31"
# MAGIC WHERE id = "73182188" OR
# MAGIC        id = "73191008" OR
# MAGIC        id = "73575322"
