...
silver_answers_df = spark.read.format("delta").load(f"/mnt/gfaproject/bronze/b_stackoverflow_answers/")
#silver_answers_df.where(to_date(col(f"last_activity_date_datetime_utc"), "yyyyMMdd") <= to_date(lit(f"{v_file_date}"), "yyyyMMdd")).show(5, False)

# COMMAND ----------

creating_time = current_timestamp()
silver_answers_df = silver_answers_df.withColumn("valid_date", to_date(lit(v_file_date), "yyyyMMdd"))
silver_answers_df = silver_answers_df.withColumn("dbx_created_at_datetiime_utc", creating_time)
silver_answers_df = silver_answers_df.withColumn("dbx_updated_at_datetime_utc", col("dbx_created_at_datetiime_utc"))
...
display(silver_answers_df)
silver_answers_df.printSchema()
...
#%sql
#DROP TABLE IF EXISTS scd.scdType1;
#CREATE TABLE scd.scdType1 USING delta AS
#SELECT employee_id, first_name, last_name, gender, address_street, address_city, address_country, email, job_title FROM scd.employees

# COMMAND ----------

if spark._jsparkSession.catalog().tableExists('silver_db', 's_stackoverflow_answers'):
    print("Table called: silver_db.s_stackoverflow_answers exist! Let's merge tables!")
    # Convert table to Delta
    deltaTable = DeltaTable.forName(spark, "silver_db.s_stackoverflow_answers")

    # Merge Delta table with new dataset
    deltaTable.alias("original1").merge(silver_answers_df.alias("updates1"),"original1._pk = updates1._pk") \
    ...
    print("Not exist, creating table called: silver_db.s_stackoverflow_answers.")
    silver_answers_df.write.mode("overwrite").format("delta").option("overwriteSchema", "true").saveAsTable("silver_db.s_stackoverflow_answers")

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT * FROM silver_db.s_stackoverflow_answers
